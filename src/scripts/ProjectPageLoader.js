...
import editButton from "../assets/edit-text.png";
...
import { CreateProjectButton, reloadProjects } from "./ProjectCardLoader";
...
export function LoadProjectView (project) {
  ...
  mainBottom.classList.add("main_bottom");

  sideBar.appendChild(backButton);

  mainTop.appendChild(title);

  mainScreen.appendChild(mainTop);
  mainScreen.appendChild(mainBottom);
...
function createTaskItem (task, i) {
  ...
  editButtonElement.src = editButton;
  ...
  editDivCont.appendChild(editButtonElement);
  ...
  mainBottom.appendChild(taskContainer);
}
...
function loadTasks () {
  ...
  for (let i = 0; i < project.tasks.length; i++) {
    try {
      ...
      if (task.title !== undefined) { createTaskItem(task, i); }
    } catch (error) {
...
function addAddButton () {
  ...
  mainBottom.appendChild(plusDiv);
  mainScreen.appendChild(mainBottom);
  content.appendChild(mainScreen);
}
